...
export default class SpaceAndTimeSDK {
    constructor() {
        this.baseUrl = process.env.REACT_APP_SXT_API_BASE_URL;
    } 
    ...
    #generateAuthCode = async function (userId, prefix, joinCode) { 
        try {
            ...
            const response = await axios.post(`${this.baseUrl}/auth/code`, payload,{headers : {"accept": "application/json"}});
    ...
    async AuthenticateUser(privateKeyArg = "", publicKeyArg = "") {
        ...
        let userId = process.env.REACT_APP_SXT_USERID;
    ...
    async Authenticate(privateKey, publicKey, prefix = "") {
        ...
        let userId = process.env.REACT_APP_SXT_USERID, joinCode = process.env.JOINCODE || "", scheme = process.env.SCHEME || "";
